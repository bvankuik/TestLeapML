...
log() {
	echo "$(date +"%Y-%m-%dT%H:%M:%S") $1" >> "$TMPDIR/TestLeapML_gen_files.log"
}
...
if [ ! -f $RESULT_FILE ]
then
	log "creating result file $RESULT_FILE"
	...
	log "Found result file $RESULT_FILE"
fi
...
if [[ "$PREV_GIT_RESULT" == "$NEW_FILES" ]]; then
	log "No changes since last git diff, do nothing"
	exit 0
fi

# Store the current git diff for the next run
echo -n "$NEW_FILES" > "$RESULT_FILE"
log "Generating new source file list"
...
do
	log "Find all .swift files in the folder ${dir} to create the xcfilelist file for the SwiftLint build script"
